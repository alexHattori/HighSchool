...
class QuitState(State):
    def __init__(self,screen):
        super(QuitState,self).__init__(screen)
class StateMachine(object):
    ...
    def run(self):
        running = True
        while running:
            self.currState.getScreen().fill(self.bgColor)
            if self.currState.isDone():
                self.currState = self.currState.getNextState()
            else:
                self.loopMethods()
            if(isinstance(self.currState,QuitState)):
                running = False
            pygame.display.flip()
            for event in pygame.event.get():
                if event.type == pygame.KEYUP:
                    self.currState.updateKey(event.key)
